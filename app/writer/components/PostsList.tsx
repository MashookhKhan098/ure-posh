...
export default function PostsList() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      // Get writer user data from localStorage
      const writerUserStr = localStorage.getItem('writerUser')
      if (!writerUserStr) {
        setError('Writer session not found. Please login again.')
        return
      }
      
      const writerUser = JSON.parse(writerUserStr)
      
      // Fetch posts from writer database
      const response = await fetch(`/api/writer/posts?writer_id=${writerUser.username}`)
      if (response.ok) {
        const data = await response.json()
        console.log('Fetched writer posts data:', data) // Debug log
        
        // Transform writer posts data to match Post interface
  ...
  const getStatusBadge = (status: string, postStatus?: string) => {
    ...
    if (postStatus) {
      ...
      const config = approvalConfig[postStatus as keyof typeof approvalConfig] || approvalConfig.pending
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      {getStatusBadge(post.status, post.post_status)}
                      {post.is_featured && (
                  ...
                  {post.post_status === 'rejected' && post.rejection_reason && (
                    <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                      <div className="flex items-center text-red-800">
                        <AlertCircle className="h-4 w-4 mr-2" />
                        <span className="font-medium">Rejection Reason:</span>
                      </div>
                      <p className="text-red-700 mt-1 text-sm">{post.rejection_reason}</p>
                    </div>
                  )}
